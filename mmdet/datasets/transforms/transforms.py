...
@jit(nopython=True)
def add_haze(img: np.ndarray, center: Tuple[int, int], size: float, beta: float, A: float):
...
class RandomFogByBeta(BaseTransform):
    def __init__(self, prob=0.5, beta_range=(0.05, 0.15), A=0.5):
        self.prob = prob
        self.beta_range = beta_range
        self.A = A

    @cache_randomness
    def _random_should_apply(self):
        return random.random() < self.prob

    @cache_randomness
    def _random_beta(self):
        return random.uniform(self.beta_range[0], self.beta_range[1])

    def transform(self, results):
        if not self._random_should_apply():
            return results
        ...
        foggy_img = add_haze(img, center, size, beta, self.A)
        foggy_img = np.clip(foggy_img * 255, 0, 255).astype(np.uint8)

        results['img'] = foggy_img
        results['img_shape'] = foggy_img.shape
        results['ori_shape'] = foggy_img.shape
...
@TRANSFORMS.register_module()
class RandomRain(BaseTransform):
    def __init__(self,
                 prob=0.5,
                 slant_range=(-10, 10),
                 drop_length_range=(10, 50),
        ...
        self.drop_length_range = drop_length_range
    ...
    def _random_params(self):
        slant1 = random.randint(*self.slant_range)
        slant2 = random.randint(*self.slant_range)
        slant_lower, slant_upper = sorted([slant1, slant2])

        return dict(
            slant_lower=slant_lower,
            slant_upper=slant_upper,
            drop_length=random.randint(*self.drop_length_range),
            drop_width=random.randint(*self.drop_width_range),
    ...
    def transform(self, results):
        ...
        # 构建当前的随机 rain transform
        rain_transform = Compose([
            albumentations.RandomRain(
                slant_lower=params['slant_lower'],
                slant_upper=params['slant_upper'],
                ...
                rain_type=params['rain_type'],
                p=1.0
            )
           # albumentations.Lambda(image=lambda x, **kwargs: np.clip(x, 0, 255).astype(np.uint8))  # clip 避免值超出
        ])

        img = results['img']
        augmented = rain_transform(image=img)
        aug_image = augmented['image']
        aug_image = np.clip(aug_image, 0, 255)
        results['img'] = aug_image
        results['img_shape'] = aug_image.shape
        results['ori_shape'] = aug_image.shape
...
@TRANSFORMS.register_module()
class LowLightSim(BaseTransform):
    """Simulate low-light conditions using nonlinear gamma-like transformation:
        output = (image / 255) ** lowlight_param * 255

    Args:
        lowlight_range (tuple): Range to sample lowlight_param from.
        prob (float): Probability of applying this transform. Default: 1.0
    """

    def __init__(self, lowlight_range=(1.5, 5.0), prob=1.0):
        assert isinstance(lowlight_range, (list, tuple)) and len(lowlight_range) == 2
    ...
    def transform(self, results: dict) -> dict:
        if random.random() < self.prob:
            assert 'img' in results, 'Input dict must contain "img" key.'
            img = results['img'].astype(np.float32)

            # Normalize to [0, 1]
            img /= 255.0

            # Sample a random lowlight parameter
            lowlight_param = random.uniform(*self.lowlight_range)

            # Apply gamma-like transformation
            img = np.power(img, lowlight_param)

            # Rescale back to [0, 255]
            img *= 255.0

            img = np.clip(img, 0, 255)  # ensure values are valid
            results['img'] = img.astype(np.float32)  # or uint8 if downstream expects that

        return results

    def __repr__(self):
        return (f'{self.__class__.__name__}('
                f'lowlight_range={self.lowlight_range}, '
                f'prob={self.prob})')


@TRANSFORMS.register_module()
class LowIlluminationDegradation(BaseTransform):
    def __init__(self, 
                 degradation_cfg,
                 safe_invert=False,
                 prob=0.5):
        self.degradation_cfg = degradation_cfg
        self.safe_invert = safe_invert
        self.prob = prob
    ...
    def random_noise_levels(self):
        ...
        log_shot = np.random.uniform(log_min_shot, log_max_shot)
        shot_noise = np.exp(log_shot)

        log_read = 2.18 * log_shot + 1.20 + np.random.normal(scale=0.26)
        read_noise = max(np.exp(log_read), 1e-6)
        return shot_noise, read_noise

    def transform(self, results):
        if random1.random() < self.prob:
            img = results['img'].astype(np.float32) / 255.0  # HWC, float32 in [0,1]
            config = self.degradation_cfg

            # --- (1) RGB to RAW ---
            img1 = 0.5 - np.sin(np.arcsin(1.0 - 2.0 * img) / 3.0)

            epsilon = 1e-8
            gamma = random1.uniform(*config['gamma_range'])
            img2 = np.clip(img1, a_min=epsilon, a_max=None) ** gamma

            xyz2cam = random1.choice(self.xyz2cams)
            rgb2cam = np.matmul(xyz2cam, self.rgb2xyz)
            rgb2cam = rgb2cam / np.sum(rgb2cam, axis=-1, keepdims=True)
            img3 = img2 @ rgb2cam.T  # (H, W, 3)

            rgb_gain = random1.normalvariate(*config['rgb_range'])
            red_gain = random1.uniform(*config['red_range'])
            blue_gain = random1.uniform(*config['blue_range'])
            gains1 = np.array([1.0 / red_gain, 1.0, 1.0 / blue_gain]) * rgb_gain

            if self.safe_invert:
                img3_gray = img3.mean(axis=2, keepdims=True)
                inflection = 0.9
                mask = np.clip((img3_gray - inflection) / (1.0 - inflection), 0.0, None) ** 2.0
                safe_gains = np.maximum(mask + (1.0 - mask) * gains1, gains1)
                img4 = np.clip(img3 * safe_gains, 0.0, 1.0)
            else:
                img4 = img3 * gains1
            ...
            mu, sigma = 0.1, 0.08
            darkness = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma).rvs()
            img5 = img4 * darkness

            # Noise
            shot_noise, read_noise = self.random_noise_levels()
            var = img5 * shot_noise + read_noise
            var = np.clip(var, a_min=epsilon, a_max=None)
            noise = np.random.normal(loc=0.0, scale=np.sqrt(var), size=img5.shape)
            img6 = img5 + noise

            # --- (3) RAW to RGB (ISP) ---
            bits = random1.choice(config['quantisation'])
            quan_noise = np.random.uniform(low=-1/(255 * bits), high=1/(255 * bits), size=img6.shape)
            img7 = img6 + quan_noise

            gains2 = np.array([red_gain, 1.0, blue_gain])
            img8 = img7 * gains2

            cam2rgb = np.linalg.inv(rgb2cam)
            img9 = img8 @ cam2rgb.T
            img10 = np.clip(img9, a_min=epsilon, a_max=None) ** (1.0 / gamma)

            img_low = np.clip(img10 * 255.0, 0.0, 255.0).astype(np.uint8)  # (H, W, C)
