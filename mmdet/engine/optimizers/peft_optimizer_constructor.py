...
class PEFTOptimWrapperConstructor(DefaultOptimWrapperConstructor):
    ...
    def add_params(
        ...
        # Calculate and log total trainable parameters in this module
        total_params = sum(param.numel() for param in module.parameters() if param.requires_grad)
        print_log(f"{prefix} total trainable parameters: {total_params}", logger="current")
    ...
    def __call__(self, model: nn.Module) -> OptimWrapper:
        ...
        if not self.paramwise_cfg:
            ...
            # Calculate and log total trainable parameters in the model
            total_trainable_params = sum(param.numel() for param_group in params for param in param_group['params'])
            print_log(f"Total trainable parameters in the model: {total_trainable_params}", logger="current")    
            print_log(model)
