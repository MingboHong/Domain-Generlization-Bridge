...
from archs.difusion_extractor_diff import DiffusionExtractorDIFF
from archs.diffusion_extractor_text import DiffusionExtractorText

from archs.diffusion_extractor_baseline import DiffusionExtractorBase
...
def load_models_stride(config, device='cuda'):
    if config.get('is_aux', False):
        print("runing DIFF with AUX")
        if config.get('with_text', False):
            diffusion_extractor = DiffusionExtractorText(config, device)
        else:
            diffusion_extractor = DiffusionExtractorDIFF(config, device)
    else:
        if config.get('baseline', False):
            diffusion_extractor = DiffusionExtractorBase(config, device)
        else:
...
class DIFFEncoder(nn.Module):
    ...
    def forward(self, img_tensor, ref_masks=None, ref_labels=None):
        ...
        #with torch.no_grad():
        def extractor_forward(*inputs):
            return self.diffusion_extractor.forward(
                inputs[0], stride_mode=True, ref_masks=inputs[1], ref_labels=inputs[2]
            )
        
        if self.config.get('is_sa', False):
            feats = cp.checkpoint(extractor_forward, img_tensor, ref_masks, ref_labels)
        else:
