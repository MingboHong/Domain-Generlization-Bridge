...
class StrideVanillaNetwork(nn.Module):
    def __init__(
        ...
        mixing_weights = [torch.ones(
            self.feature_cnts[i] * len(save_timestep)) * 0.01 for i in range(self.num_stride)]
        self.mixing_weights_stride = nn.ParameterList(
            [nn.Parameter(mixing_weights[i].to(device)) for i in range(self.num_stride)])

        self.in_stride = nn.ParameterList([nn.InstanceNorm2d(
            feature_dims[i], affine=True) for i in range(len(feature_dims))])
    ...
    def forward(self, batch_list):
        ...
        mixing_weights_stride = [torch.nn.functional.softmax(
            self.mixing_weights_stride[i], dim=0) for i in range(self.num_stride)]
        ...
        for idx_i in range(len(self.feature_stride_idx)):
            for timestep_i in range(len(self.save_timestep)):
                ...
                start_channel = instride_num * \
                    len(self.save_timestep) * num_channel + \
                    timestep_i * num_channel
                feats = batch_list[stride_id][:,
                                              start_channel:start_channel+num_channel, :, :]

                feats = self.in_stride[idx_i](feats)

                feats = mixing_weights_stride[stride_id][timestep_i *
                                                         self.feature_cnts[stride_id] + instride_num] * feats
        ...
        for stride_id in range(self.num_stride):
            bottleneck_layer = nn.Sequential(
                *ResNet.make_stage(
                    ...
                    in_channels=feature_dims_by_stride[stride_id] *
                    len(save_timestep),
                    bottleneck_channels=projection_dim[stride_id],
...
class StrideAggregationNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.bottleneck_layers = self.bottleneck_layers.to(device)
        mixing_weights = [torch.ones(
            self.feature_cnts[i] * len(save_timestep)) for i in range(self.num_stride)]
        self.mixing_weights_stride = nn.ParameterList(
            [nn.Parameter(mixing_weights[i].to(device)) for i in range(self.num_stride)])
    ...
    def forward(self, batch_list):
        ...
        for idx_i in range(len(self.feature_stride_idx)):
            ...
            for timestep_i in range(len(self.save_timestep)):
                ...
                bottlenecked_feature = mixing_weights_stride[stride_id][timestep_i *
                                                                        self.feature_cnts[stride_id] + instride_num] * bottlenecked_feature
                if output_features[stride_id] is None:
...
class AggregationNetwork(nn.Module):
    ...
    def __init__(
        ...
        for l, feature_dim in enumerate(self.feature_dims):
            ...
            for t in save_timestep:
                ...
                self.mixing_weights_names.append(
                    f"timestep-{save_timestep}_layer-{l+1}")

        self.bottleneck_layers = self.bottleneck_layers.to(device)
        mixing_weights = torch.ones(
            len(self.bottleneck_layers) * len(save_timestep))
    ...
    def forward(self, batch):
        ...
        for i in range(len(mixing_weights)):
            # Share bottleneck layers across timesteps
            bottleneck_layer = self.bottleneck_layers[i % len(
                self.feature_dims)]
            # Chunk the batch according the layer
            # Account for looping if there are multiple timesteps
