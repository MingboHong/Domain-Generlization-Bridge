...
from archs.clip import ClipAdapter
from timm.models.layers import trunc_normal_
from torch.utils.checkpoint import checkpoint
from archs.cim_utils import FrontDoorBasisBlock
# from models.cim_utils import MultiScaleBasisBlock
from archs.stable_diffusion.blocks import LightAdapter

class PositionalLinear(nn.Module):
    def __init__(self, in_features, out_features, seq_len=77, clip_fdblock=True, bias=True,basis_reduction=2, basis_reduction_mode="sub", with_ep = True,with_query = True, with_mssp=False, **kwargs):
        super().__init__()
        self.clip_fdblock = clip_fdblock
        if self.clip_fdblock:
            self.fdblock = FrontDoorBasisBlock(feat_dim=out_features,basis_reduction=basis_reduction,
                                               basis_reduction_mode=basis_reduction_mode,with_ep=with_ep, with_query= with_query,with_mssp= with_mssp, **kwargs)

        self.global_linear = nn.Linear(in_features, out_features, bias=bias)
        self.local_linear1 = nn.Linear(in_features, out_features, bias=bias)
        self.local_linear2 = nn.Linear(14*14, seq_len, bias=bias)
        self.positional_embedding = nn.Parameter(torch.zeros(1, seq_len, out_features))
        self.gamma =  nn.Parameter(torch.ones(2))
        trunc_normal_(self.positional_embedding, std=0.02)

        nn.init.zeros_(self.local_linear2.weight)
        if self.local_linear2.bias is not None:
            nn.init.zeros_(self.local_linear2.bias)

    def forward(self, xg, xl):

        xg = self.global_linear(xg)
        xg = xg.unsqueeze(1) + self.positional_embedding
        B, C, H, W = xl.shape
        xl = xl.reshape(B, C, -1).permute(0, 2, 1)
    
        xl = self.local_linear1(xl)
        if self.clip_fdblock:
            xl = self.fdblock(xl)
        xl = xl.permute(0, 2, 1)
        xl = self.local_linear2(xl).permute(0, 2, 1).contiguous()
        out = xg * self.gamma[0] + xl * self.gamma[1]
...
class DiffusionExtractor(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.pipe, self.unet, self.vae = init_models(
       ...
       # del self.clip, self.clip_tokenizer
        ...
        self.with_loss = config.get("with_loss", False)
        ...
        self.clip = ClipAdapter(name ='ViT-L-14',normalize=False).to(self.unet.device)

        clip_dim = self.config.get("clip_dim", 1024)
        self.clip_project = PositionalLinear(self.clip.dim_latent, clip_dim, 77, config.get("clip_fdblock"), basis_reduction=config.get("clip_basis_reduction",2), 
                                            basis_reduction_mode=config.get("basis_reduction_mode", "div"),
                                            with_ep = config.get("with_ep", True), with_query = config.get("with_query", True), 
                                            with_mssp = config.get("with_mssp", False)).to(self.unet.device)
        ...
        if config.get("unet_plugin", False):
            is_fdblock = config.get("plugin_fdblock", True)
            self.cond_net = LightAdapter(in_channels=4, is_fdblock=is_fdblock, basis_reduction=config.get("plugin_basis_reduction",2), 
                                         basis_reduction_mode=config.get("basis_reduction_mode", "div"),with_ep = config.get("with_ep", True),
                                         with_query = config.get("with_query", True), 
                                            with_mssp = config.get("with_mssp", False)
                                         )
            #self.fd_unet = MultiScaleBasisBlock(in_channels=1280)
            for name, param in self.cond_net.named_parameters():
                    print(f"{name}: requires_grad = {param.requires_grad}")

    def set_cond_from_image(self, images):
        images = (images + 1) / 2  # [-1,1] -> [0,1]
        with torch.autocast("cuda"):
            clip_feats = self.clip.embed_image(images)
            global_feats = clip_feats.image_embed  
            local_feats  = clip_feats.image_encodings
            global_feats = self.clip_project(global_feats, local_feats) 
        self.cond[self.batch_size] = global_feats
        self.uncond[self.batch_size] = global_feats
    ...
    def run_inversion(self, latent, mlvl_feats= None,ref_masks=None, ref_labels=None, images=None, min_i=None, max_i=None,
                      mode="norm"):
        if mode == "norm":
            xs = generalized_steps(
                latent,
    ...
    def get_feats_stride(self, latents, mlvl_feats, extractor_fn, preview_mode=False, do_optim_steps=False):
        ...
        outputs = extractor_fn(latents, mlvl_feats)
        if not preview_mode:
    ...
    def forward(self, images=None, latents=None, mlvl_feats = None,guidance_scale=-1, preview_mode=False, stride_mode=False, ref_masks=None, ref_labels=None):
        if images is None:
            if latents is None:
            ...
            if self.diffusion_mode == "generation":
                if preview_mode:
                    def extractor_fn(latents, mlvl_feats): return self.run_generation(
                        latents, mlvl_feats, guidance_scale, max_i=self.end_timestep)
            elif self.diffusion_mode == "inversion":
                raise NotImplementedError
        else:
            self.set_cond_from_image(images)
            latents = self.vae.encode(images).latent_dist.sample(
                generator=None) * 0.18215
            if self.config.get("unet_plugin", False):
                mlvl_feats = self.cond_net(latents.float())
            else:
                mlvl_feats = None
            if self.diffusion_mode == "inversion":
                def extractor_fn(latents, mlvl_feats): return self.run_inversion(
                    latents, mlvl_feats, ref_masks=ref_masks, ref_labels=ref_labels, images=images)
        ...
        with torch.autocast("cuda"):
            if stride_mode:
                return self.get_feats_stride(latents, mlvl_feats, extractor_fn)
