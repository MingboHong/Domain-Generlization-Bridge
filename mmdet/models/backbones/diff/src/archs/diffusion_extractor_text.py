...
from archs.stable_diffusion.diffusion import (
    init_models,
    init_models_with_text,
    get_tokens_embedding,
    generalized_steps,
    generalized_steps_mask,
...
#     def __init__(self, in_features, out_features, seq_len=77, clip_fdblock=True, bias=True,**kwargs):
#         super().__init__()
#         self.clip_fdblock = clip_fdblock
#         if self.clip_fdblock:
#             self.fdblock = FrontDoorBasisBlock(in_channels=out_features,**kwargs)
...
class DiffusionExtractorText(nn.Module):
    """
    ...
    def __init__(self, config, device):
        ...
        self.pipe, self.unet, self.vae, self.clip, self.clip_tokenizer = init_models_with_text(
       ...
       # del self.clip, self.clip_tokenizer
        ...
        # self.clip_project = PositionalLinear(self.clip.dim_latent, 1024, 77, config.get("clip_fdblock"))
        ...
        if config.get("fdblock_unet"):
            self.cond_net = LightAdapter(in_channels=4)
        ...
        if images is None:
            ...
            if ref_labels and ref_labels:
               ...
               # self.set_cond_from_image(images)
                ...
                mlvl_feats_images = self.cond_net(latents_image.float())
                ...
                latents_mask_images = self.vae.encode(mask_images).latent_dist.sample(
                    generator=None) * 0.18215
                mlvl_feats_images_mask = self.cond_net(latents_mask_images.float())
                mlvl_feats.append(mlvl_feats_images)
                mlvl_feats.append(mlvl_feats_images_mask)
