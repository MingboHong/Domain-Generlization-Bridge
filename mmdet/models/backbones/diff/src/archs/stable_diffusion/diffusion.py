...
def get_xt_next(xt, et, at, at_next, a_skip, eta, tmask, do_adpm_steps=False, gamma_t=None):
    ...
    c2 = torch.max((1 - at_next) - (c1 * tmask) ** 2,
                   torch.Tensor([1e-10]).to(et.device))[0].sqrt()
    ...
    if do_adpm_steps and eta > 0.0:
        ...
        coeff_cov_x_0 = (at_next ** 0.5 - ((c2 ** 2)
                         * at / (1 - at)) ** 0.5) ** 2
...
def generalized_steps(x, model, scheduler, **kwargs):
    ...
    b = scheduler.betas.to(x.device)

    batch_size = x.shape[0]

    mlvl_feats = kwargs.get("mlvl_feats", None)
    seq_next = list(seq[:-1])
    ...
    for i, (t, next_t) in enumerate(zip(seq_iter, seq_next_iter)):
        ...
        t_tensor = (torch.ones(batch_size) * t).to(x.device)
        next_t_tensor = (torch.ones(batch_size) * next_t).to(x.device)

        at = (1 - b).cumprod(dim=0).index_select(0, t_tensor.long())
        at_next = (1 - b).cumprod(dim=0).index_select(0, next_t_tensor.long())

        # Expand dims for broadcasting
        at = at[:, None, None, None]
        at_next = at_next[:, None, None, None]

        # Set model timestep
        if do_inversion:
            set_timestep(model, len(seq_next) - i - 1)
        else:
        ...
        if guidance_scale == -1:
            ...
            et = model(xt, t_tensor, encoder_hidden_states=cond,down_intrablock_additional_residuals=mlvl_feats).sample
        else:
            uncond = kwargs["unconditional"]
        
            et_uncond = model(xt, t_tensor, encoder_hidden_states=uncond,down_intrablock_additional_residuals=mlvl_feats).sample
            et_cond = model(xt, t_tensor, encoder_hidden_states=cond,down_intrablock_additional_residuals=mlvl_feats).sample
            et = et_uncond + guidance_scale * (et_cond - et_uncond)

        eta = kwargs.get("eta", 0.0)
        s_tmin = kwargs.get("s_tmin", 10)
        s_tmax = kwargs.get("s_tmax", 250)
        # Apply eta only in certain timestep range
        if t > s_tmin and t < s_tmax:
            eta_val = eta
        else:
            eta_val = 0.0

        # Calculate a_skip ratio for step size
        a_skip = at / at_next if t > next_t else at_next / at

        # Use the standard DDIM update with eta noise
        noise = torch.randn_like(et)
        x0_t = (xt - et * (1 - at).sqrt()) / at.sqrt()
        xt_next = (
            at_next.sqrt() * x0_t
            + eta_val * ((1 - a_skip) * (1 - at_next) / (1 - at)).sqrt() * noise
            + ((1 - at_next) - (eta_val**2) * (1 - a_skip) * (1 - at_next) / (1 - at)).sqrt() * et
        )
...
def generalized_steps_baseline(x, model, scheduler, **kwargs):
    ...
    for i, (t, next_t) in enumerate(zip(seq_iter, seq_next_iter)):
        ...
        if guidance_scale == -1:
            ...
            et = model(xt, t_tensor,encoder_hidden_states=cond).sample
        else:
            uncond = kwargs["unconditional"]
            xt_input = xt_input.repeat(2, 1, 1, 1)
            encoder_hidden_states = torch.cat([uncond, cond], dim=0)
            et = model(
                xt_input, t, encoder_hidden_states=encoder_hidden_states).sample
...
def generalized_steps_mask(x, model, scheduler, **kwargs):
    """
    Performs either the generation or inversion diffusion process.
    """
    ref_masks = kwargs.get("ref_masks", None)
    ref_labels = kwargs.get("ref_labels", None)
    mlvl_feats = kwargs.get("mlvl_feats", None)
    if ref_masks is not None:
        x, mask_x = x
        mlvl_feats, mlvl_feats_mask = mlvl_feats
    seq = scheduler.timesteps
    seq = torch.flip(seq, dims=(0,))
    b = scheduler.betas
    b = b.to(x.device)
   ...
   # images = kwargs.get("images", None)
    ...
    with torch.no_grad():
        ...
        #     t = torch.tensor(seq_iter, dtype=torch.long, device=x.device)
        #     noise = torch.randn_like(x).to(x.device)
        #     x = scheduler.add_noise(x, noise, t)
        
        
        # #  add noise first
        # t = torch.tensor(seq_iter[0], dtype=torch.long, device=x.device)
        # noise = torch.randn_like(x).to(x.device)
        # x = scheduler.add_noise(x, noise, t)
        # print(t)
        ...
        tmask = torch.ones((x.shape[2], x.shape[3])).to(
        ...
        if do_mask_steps:
            ...
            masks = torch.ones((1, batch_size, 1, x.shape[2], x.shape[3])).to(
                dtype=x.dtype, device=x.device)
            prompts = []
            prompt_cnt = 0
            if ref_masks != None and ref_labels != None:
                prompts += ref_labels
                mask = ref_masks.to(dtype=x.dtype, device=x.device)
                mask = mask[:, None, :, :]
                mask = F.interpolate(mask, size=(x_h, x_w), mode='nearest')
                mask_ = mask[None, ...]
                masks = torch.cat([masks, mask_], dim=0)
                prompt_cnt += 1
                _, label_embedding = get_tokens_embedding(
                    clip_tokenizer, clip, x.device, prompts)
                uncond_embedding =  kwargs["unconditional"]
                text_embedding = torch.cat(
                    [uncond_embedding, label_embedding], dim=0)
        ...
        for i, (t, next_t) in enumerate(zip(seq_iter, seq_next_iter)):
            ...
            if ref_masks != None and ref_labels != None:
                # xt_input = torch.cat([xt_input, mask_x], dim=0)
                # et = model(
                #     xt_input, t, encoder_hidden_states=text_embedding).sample
                # et_uncond = et[:batch_size, ...].repeat(prompt_cnt, 1, 1, 1)
                # et_text = et[batch_size:, ...]      
                # et = et_uncond + guidance_scale * (et_text - et_uncond)
                et = model(mask_x, t, encoder_hidden_states=label_embedding,down_intrablock_additional_residuals=mlvl_feats_mask).sample

            elif guidance_scale == -1:
                # print('ok2')
                et = model(xt, t, encoder_hidden_states=cond,down_intrablock_additional_residuals=mlvl_feats).sample
            else:
                # print('ok3')
                ...
                encoder_hidden_states = torch.cat([uncond, cond], dim=0)
                et = model(
                    xt_input, t, encoder_hidden_states=encoder_hidden_states,down_intrablock_additional_residuals=mlvl_feats).sample
                et_uncond = et[:batch_size, ...]
                et_cond = et[batch_size:, ...]
                et = et_uncond + guidance_scale * (et_cond - et_uncond)
            ...
            if do_mask_steps and ref_masks != None and ref_labels != None:
                ...
                value = (xts_next * mask_.view(prompt_cnt*batch_size, 1, xts.shape[2], xts.shape[3])).view(
                    prompt_cnt, batch_size, x_c, x_h, x_w).sum(dim=0)
                # [prompt_cnt, batch_size, 1, h, w] -> [batch_size, 1, h, w]
                count = mask_.sum(dim=0)
                xt_next = torch.where(count > 0, value / count, value)
                
            else:
                _, xt_next = get_xt_next(
                    xt, et, at, at_next, a_skip, eta, tmask)
...
def generalized_steps_diff(x, model, scheduler, **kwargs):
    """
    Performs either the generation or inversion diffusion process.
    """
    ref_masks = kwargs.get("ref_masks", None)
    ...
    with torch.no_grad():
        ...
        if do_mask_steps:
            ...
            if ref_masks != None and ref_labels != None:
            ########################################################################################3
                # for label_index in range(len(ref_labels[0])):
                #     mask = torch.tensor(np.array([mask[label_index] for mask in ref_masks]),dtype=x.dtype, device=x.device) 
                #     #[batch_size, H, W] -> [batch_size, 1 ,H, W]
                #     mask = mask[:, None, :, :]
                #     mask = F.interpolate(mask, size=(x_h, x_w), mode='nearest')
                #     mask = mask[None, ...]  # [1, batch_size, 1, h, w]
                #     # final [C+1, batch_size, 1, h, w]
                #     masks = torch.cat([masks, mask], dim=0)
                #     prompts += [label[label_index] for label in ref_labels]             
                #     prompt_cnt += 1
        ...
        for i, (t, next_t) in enumerate(zip(seq_iter, seq_next_iter)):
            ...
            if ref_masks != None and ref_labels != None:
                ...
                # et_text = et[batch_size:, ...]      
                # et = et_uncond + guidance_scale * (et_text - et_uncond)
                et = model(mask_x, t, encoder_hidden_states=label_embedding).sample
...
def init_models_with_text(
    ...
    from archs.stable_diffusion.net import UNet


    my_unet = UNet.from_config(pipe.unet.config)
    my_unet.load_state_dict(pipe.unet.state_dict(), strict=True)
    unet = my_unet
    vae = pipe.vae
...
def collect_stride_feats_with_timesteplist(unet, idxs_resnet, idxs_ca, timestep_list, do_mask_steps=False,
    ...
    feats_cat_resnet_idxs = [torch.cat(feat_t, dim=1)
                             for feat_t in latent_feats_resnet_idxs]
    ...
    for idx, feat in zip(idxs_ca, latent_feats_ca):
        ...
        for timestep in timestep_list:
            ...
            feat_t = feat_t.view(prompt_cnt, batch_size,
                                 latent_h[idx[0]], latent_w[idx[0]], feat_c)
            feat_t = feat_t.permute(1, 0, 4, 2, 3)
            feat_t = feat_t.sum(dim=1)
            latents_feats_idxs_t.append(feat_t)
        latent_feats_ca_idxs.append(latents_feats_idxs_t)
    feats_cat_ca_idxs = [torch.cat(feat_t, dim=1)
                         for feat_t in latent_feats_ca_idxs]
    ...
    # Only support idxs (0,0) (0,1) (0,2) (1,0) (1,1) (1,2) (2,0) (2,1) (2,2) (3,0) (3,1) (3,2) now. Added by  
