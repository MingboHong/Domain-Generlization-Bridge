...
class DinoVisionTransformer(BaseModule):
    def __init__(
        ...
        """
        super(DinoVisionTransformer,self).__init__(init_cfg)
        ...
        if self.is_fdblock:
            ...
            for i in range(len(out_indices)):
                self.fdblocks.append(MultiScaleBasisBlock(in_channels=embed_dim,basis_reduction=basis_reduction,basis_reduction_mode = basis_reduction_mode,
                                                        with_ep = with_ep,with_query=with_query,with_ssp=with_ssp,with_mssp=with_mssp))

        # initialize weights
        named_apply(
            partial(self._init_weights, init_values=init_values), self, depth_first=True
        )
    ...
    def _init_weights(self, module: nn.Module, name: str = "", init_values=None):
        # Standard ViT / DINO style initialization
    ...
    def forward(self, *args, **kwargs):
        if self.use_checkpoint:
            out = checkpoint(self.forward_features, *args, **kwargs)
        else:    
            out = self.forward_features(*args, **kwargs)
        
        if self.is_fdblock:
            ret = []
            for x, block in zip(out, self.fdblocks):
                x = block(x)
                ret.append(x)
        else:
            ret = out
