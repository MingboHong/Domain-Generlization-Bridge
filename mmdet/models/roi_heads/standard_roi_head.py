...
class StandardRoIHead(BaseRoIHead):
    ...
    def predict_bbox(self,
        ...
        # inds = result_list[0].inds
        # bbox_feat = bbox_feats[inds]
        # result_list[0].set_field(bbox_feat, 'bbox_feats')  
        #==============new==============
        return result_list
...
@MODELS.register_module()
class CIMStandardRoIHead(StandardRoIHead):
    """Simplest base roi head including one bbox head and one mask head."""
    def __init__(self,
                 ...
                 basis_reduction: int = 2,
                 basis_reduction_mode: str = 'sub',
                 with_ssp: bool = True,
                 with_query: bool = True,
                 with_input_subspace: bool = False,
                 with_dropout: bool = False,
                 basis_normalize: bool = True,
        ...
        self.fdblock = MultiScaleBasisBlock(in_channels=bbox_roi_extractor['out_channels'], norm_cfg=None, basis_reduction=basis_reduction,basis_reduction_mode=basis_reduction_mode,
                                            with_ssp=with_ssp, with_query=with_query,
                                            with_input_subspace=with_input_subspace,
                                            with_dropout=with_dropout, basis_normalize=basis_normalize,
                                            conv_kernel_size=conv_kernel_size)
       # print(basis_reduction)
    def init_bbox_head(self, bbox_roi_extractor: ConfigType,
                       bbox_head: ConfigType) -> None:
        """Initialize box head and box roi extractor.
    ...
    def _bbox_forward(self, x: Tuple[Tensor], rois: Tensor) -> dict:
        ...
        if self.with_shared_head:
            bbox_feats = self.shared_head(bbox_feats)
        bbox_feats = self.fdblock(bbox_feats)
        cls_score, bbox_pred = self.bbox_head(bbox_feats)
